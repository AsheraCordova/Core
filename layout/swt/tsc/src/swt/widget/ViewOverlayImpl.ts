...
export abstract class ViewOverlayImpl<T> extends ViewImpl<T>{
	...
	@decorate(Type(() => CommandAttr))
	@decorate(Expose({ name: "swtUseGC" }))
	swtUseGC!:CommandAttr<boolean>| undefined;
	@decorate(Type(() => CommandAttr))
	...
	public setSwtUseGC(value : boolean) : T {
		this.resetIfRequired();
		if (this.swtUseGC == null || this.swtUseGC == undefined) {
			this.swtUseGC = new CommandAttr<boolean>();
		}
		
		this.swtUseGC.setSetter(true);
		this.swtUseGC.setValue(value);
		this.orderSet++;
		this.swtUseGC.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
