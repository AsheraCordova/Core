...
export abstract class PopupWindowImpl<T> extends ViewImpl<T>{
	...
	@decorate(Type(() => CommandAttr))
	@decorate(Expose({ name: "popupBackground" }))
	popupBackground!:CommandAttr<string>| undefined;
	@decorate(Type(() => CommandAttr))
	@decorate(Expose({ name: "popupElevation" }))
	popupElevation!:CommandAttr<string>| undefined;
	@decorate(Type(() => CommandAttr))
	...
	public setPopupElevation(value : string) : T {
		this.resetIfRequired();
		if (this.popupElevation == null || this.popupElevation == undefined) {
			this.popupElevation = new CommandAttr<string>();
		}
		
		this.popupElevation.setSetter(true);
		this.popupElevation.setValue(value);
		this.orderSet++;
		this.popupElevation.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
