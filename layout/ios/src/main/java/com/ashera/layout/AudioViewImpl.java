...
public java.util.Map<String, Object> getOnErrorEventObj(MediaPlayer mp,int what,int extra) {
	...
	private native void createAVPlayer()/*-[
	...
	private void setDataSource(Object objValue) {
		...
		if (audioPath.startsWith("http:") || audioPath.startsWith("https:") || audioPath.startsWith("content:")) {
			nativeSetAudioPathRemote(audioPath);
			...
			nativeSetAudioPathLocal(cordovaFileUri);
			...
			nativeSetAudioPathReource("raw/" + audioPath);
		}
	}
	
	private native void nativeSetAudioPathReource(String path)/*-[
	...
	private native void nativeSetAudioPathLocal(String path)/*-[
		NSURL *newURL = [NSURL fileURLWithPath:path];
    	[self replaceItemWithURL: newURL player:((AVPlayer*) self->avplayer_)];
	]-*/;

	private native void nativeSetAudioPathRemote(String path)/*-[
